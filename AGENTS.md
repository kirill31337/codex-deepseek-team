...
- Full-access workers use owned isolated development copies, may change any project files and run local tests/builds. The coordinator prepares copies/dependencies. Legacy `--write --allow-write` remains exact-file and clean-linked-worktree only.
...
- Legacy jobs retain the hybrid native Codex / outer Claude boundary. Managed development copies use a sparse outer Bubblewrap namespace for both, a read-only Git directory and a private network with only a fixed provider relay; never launch the managed runtime command outside that boundary.
...
- Workers must not stage, commit, push, publish or deploy. Read-only jobs cannot write; legacy exact-file writers cannot run tests/builds. Full-access permits local checks in its owned copy. No automatic writer retries; inspect partial work before explicit continuation.
