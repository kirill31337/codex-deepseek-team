...
- `--write` requires a clean, dedicated `codex/` linked worktree and an exact `--allow-write` file list.
...
- Fall back to coordinator work after a completed runner error, unavailable credentials/runner, or `CODEX_DEEPSEEK_DISABLED=1`; do not repeatedly call a broken provider.
- Review the actual diff, including new files; workers return a short summary instead of copied code.
- Run a worker with: `codex-deepseek-team worker`
